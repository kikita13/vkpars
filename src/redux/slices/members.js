  ...
  async (props) => {
    ...
    `;
      ...
      $.ajax({
        ...
        data: {
          code: `var account; if (${props} < 0) {account = API.groups.getById({"group_id": '${-props}', "fields": '${
            FIELDS.group
          }'});} else {account = API.users.get({"user_ids": '${props}', "fields": '${
            FIELDS.user
          }'});}; ${code}; return {items: items, count: length, account: account};`,
          access_token: TOKEN,
          v: "5.131",
        },
        dataType: "jsonp",
