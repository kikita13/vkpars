...
export const commentsRequests = (countsComms) => {
  ...
  for (let i in countsComms) {
    if (countsComms[i].count < 100) {
      codesForComments.push(`var comments = API.wall.getComments({"owner_id":'${countsComms[i].owner_id}',"post_id": '${countsComms[i].post_id}',"count":'${countsComms[i].count}', "extended": 1, "fields": '${FIELDS.user}'}); allComments.push(comments);`);
