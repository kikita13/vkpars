...
export const threadRequests = (countThreads) => {
  ...
  for (let i in countThreads) {
    if (countThreads[i].count < 100) {
      codesForThreads.push(`var thr = API.wall.getComments({"owner_id":'${countThreads[i].owner_id}',"post_id": '${countThreads[i].post_id}',"count":'${countThreads[i].count}',"comment_id": '${countThreads[i].comment_id}', "extended": 1, "fields": '${FIELDS.user}'}); threads.push(thr);`);
