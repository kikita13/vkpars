...
export const responsePosts = async (props) => {
  const codeForAccount = `var account; if (${
    props.id
  } < 0) {account = API.groups.getById({"group_id": '${-props.id}', "fields": '${
    FIELDS.group
  }'});} else {account = API.users.get({"user_ids": '${props.id}', "fields": '${
    FIELDS.user
  }'});};var profiles = [];var items = [];var groups = [];`;
  const codesForPosts = useCodePosts(
    props.maxPosts,
    props.id,
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        $.ajax({
          ...
          data: {
            code: `${codeForAccount}${chunk.join(
              ""
