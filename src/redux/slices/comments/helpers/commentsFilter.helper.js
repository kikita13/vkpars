...
export const commentsFilter = (arr, keywords, ageLess, ageOver, cities, firstNames, lastNames) => {
  ...
  ?.filter( item => lastNames  ? lastNames.some(word => item.user?.last_name?.toLowerCase().includes(word))   : true)
  ?.filter( item => firstNames ? firstNames.some(word => item.user?.first_name?.toLowerCase().includes(word)) : true)
  return filteredArr
}